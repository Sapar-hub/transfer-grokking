...
\begin{abstract}
  ...
  indistinguishable from Phi-2's unpatched baseline.  This discontinuity---a
  hard override rather than a smooth tradeoff---is exact across all five seeds
  and both operations, and follows from a layernorm-induced norm mismatch
...
\begin{figure}[ht]
  ...
  flat through $\alpha = 0.7$, then jump discontinuously at $\alpha = 1.0$---a
  hard override, not a gradual tradeoff.}
  \label{fig:alpha_add}
\end{figure}
...
($0.235$) through $\alpha = 0.7$, then jumps discontinuously to $1.0$ only at
full replacement---exact across all five seeds (42--46), with negligible
variance ($\pm 0.002$) only at $\alpha = 0.7$.
...
$\alpha \leq 0.7$, then jumps to $1.0$ at $\alpha = 1.0$, exact across all
...
\item \textbf{L31 injection:} $\alpha=1.0$ achieves $1.0$ accuracy for both
  ...
  ($0.235$ add, $0.030$ mult), confirming the discontinuous-override
...
discontinuous-override behavior---flat at baseline through $\alpha=0.7$,
...
output once it fully overrides the residual stream at $\alpha = 1.0$, at which
point language-modeling quality collapses by 7--8 orders of magnitude
(Table~\ref{tab:seed_ppl}).  This is not a smooth accuracy/fluency tradeoff,
but a consequence of training $W_{\text{CE}}$ through a scale-invariant final
layernorm: the cross-entropy objective has no incentive to match the norm of
Phi-2's own residual stream, so weight decay collapses $W_{\text{CE}}$'s
output to a norm roughly 100--200$\times$ smaller than the target activation.
...
\begin{figure}[ht]
  ...
  random-adjacent baseline ($0.030$) through $\alpha=0.7$, then jumps
  discontinuously to $1.0$ at $\alpha=1.0$---identical to addition.}
...
\begin{figure}[ht]
  ...
  remain statistically at their respective baselines---the discontinuous
  override pattern is operation-agnostic.}
...
consistently, the discontinuous-override pattern reported in Section~5 held

...
of $W_{\text{CE}}$'s output; weight decay drives it toward a small norm
 ($\|W_{\text{CE}}(h)\| \approx 0.32$ vs.\ $\|h_{\text{Phi-2}}\| \approx 225.8$
at L31, a ratio of $0.0014$).  At any $\alpha < 1.0$ this small-norm signal is
...
five seeds.  The norm ratio for multiplication
 ($\|W_{\text{CE}}(h)\| \approx 0.25$ vs.\ $\|h_{\text{Phi-2}}\| \approx 225.8$,
ratio $\approx 0.0011$) is consistent with addition's, confirming the
...
both values are within noise of the unpatched baseline ($0.235$), and the
difference is not meaningful.
This confirms that the injection depth---not the projection quality---was the
...
It is important to note that the $\alpha=1.0$ result is partially
guaranteed by construction: $W_{\text{CE}}$ is trained through the same
frozen \texttt{final\_layernorm} $\rightarrow$ \texttt{lm\_head} path used
at L31 evaluation, so the $\alpha=1.0$ accuracy reproduces the logit-lens
accuracy from Section~4 rather than providing independent evidence.
The informative content of the L31 injection experiment is therefore the
layer dependence ($\text{L31} \gg \text{L10}$) and the norm-mismatch
mechanism governing intermediate $\alpha$, not the $\alpha=1.0$ number in
isolation.
...
at which point language-modeling perplexity rises by a modest 25\% (PPL~99.8

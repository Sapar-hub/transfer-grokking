...
\begin{abstract}
  ...
  layer (L31 for Phi-2) achieves perfect task accuracy ($1.0$) with only mild
  perplexity degradation at intermediate mixing strengths.
...
$(a \times b) \bmod 97$, grokking at epoch~10,179.
...
identical activations under different prompts produce stable geometry but
only partially separable arithmetic structure.
...
\begin{figure}[ht]
  ...
  Shaded regions: $\pm 1$ std over 5 seeds.  Accuracy rises monotonically
...
\begin{itemize}[nosep]
\item \textbf{Grokking:} the small model groks multiplication at epoch~10,179
  (vs.\ earlier for addition), confirming the training protocol extends to
...
property of encoding all 0--96 as single tokens was essential for
...
(\texttt{"\# ({a} + {b}) \% 97 ="}) achieves $0.41$ accuracy---above
random ($0.01$) but well below the grokked model's $1.0$.
This confirms that Phi-2's L31 activations encode stable syntactic features
rather than general arithmetic structure.  With mixed templates, accuracy
drops to $0.04$, demonstrating extreme template sensitivity.

# Findings — Grokking Geometry Transfer Experiments

Phase-by-phase narrative and complete list of findings from the research.
...
W_CE at L31 achieves **perfect accuracy (1.0) at α=1.0**; at coarse 5-point resolution (α∈{0.0, 0.3, 0.5, 0.7, 1.0}), intermediate α appears flat at baseline. Fine-resolution analysis (73 α points, n=2823) reveals a continuous S-curve: 86% of non-baseline examples flip within Δα ≈ 0.05 (α∈[0.95, 1.0]), with zero non-monotonic transitions. The old no-layernorm W_CE numbers (α=0.7→0.995, etc.) were an artifact of training in the wrong representational space. W_MSE at L31 does nothing (all α = baseline 0.235).

**Key findings:**
1. **CE through lm_head resolves barrier 1:** W_CE logit lens = 1.0, proving MSE was misaligning W from decoding directions
2. **The "discontinuous override" was a coarse-grid artifact — the true transition is a sharp-threshold S-curve.** At 5-point resolution (α∈{0.0, 0.3, 0.5, 0.7, 1.0}), W_CE appears flat at baseline (0.235) through α=0.7, then jumps to 1.0 at α=1.0 — an apparent step. Fine-resolution analysis (73 α points, n=2823, seeds 42–46) reveals the transition is continuous: 0.281@α=0.7, 0.318@α=0.9, 0.496@α=0.98, 0.719@α=0.99, 0.983@α=0.995, 1.0@α=0.999, with 86% of non-baseline examples flipping within Δα ≈ 0.05 and zero non-monotonic transitions. The old no-layernorm numbers (α=0.7→0.995, etc.) reflected W_CE operating in the wrong representational space (see Barrier 1, main paper §4). Under the architecturally correct layernorm-inclusive objective, the mechanism remains the same — norm mismatch from layernorm-inclusive training — but the transition is a steep continuous S-curve, not a discrete cliff.
...
- The mechanism is a **norm-mismatch-induced sharp threshold**: W_CE's output norm is ~0.7% of Phi-2's residual norm, making the injected signal negligible at α<1.0 except within Δα ≈ 0.05 of full replacement, where the continuous S-curve resolves as the injected state begins to dominate
...
10. **Neural function call works: inject grokked state at L31 → perfect accuracy at α=1.0.** W_CE trained through the architecturally correct layernorm-inclusive objective achieves logit-lens accuracy = 1.0 and, injected at full strength at L31, perfect task accuracy (1.0) on both operations. The transition from baseline to perfect accuracy is a continuous S-curve compressed into Δα ≈ 0.05 (86% of non-baseline examples flip within α∈[0.95, 1.0]), which the original 5-point grid sampled as an apparent step. The "discontinuous override" of v2.0.0 was a measurement-resolution artifact. The mechanism is a norm mismatch (W_CE output norm ≈ 0.7% of Phi-2's residual norm) that shifts the transition to α values near 1.0. Grokked representations are transferable, but the injection-strength response is a sharp threshold, not a blended tradeoff.
11. cos_sim between different-dim residual streams plateaus at ~0.30 regardless of conditioning
12. **L31 injection accuracy — corrected from v2.0.0.** The transition is a continuous S-curve compressed into Δα ≈ 0.05, not a discontinuous cliff. At coarse 5-point resolution (α∈{0.0, 0.3, 0.5, 0.7, 1.0}, n=2823), accuracy appears flat at baseline (0.235 add, 0.030 mult) then jumps to 1.0 at α=1.0. Fine-resolution analysis (73 α points, 5 seeds) resolves the intermediate values: 0.281@α=0.7, 0.318@α=0.9, 0.496@α=0.98, 0.719@α=0.99, 0.983@α=0.995, 1.0@α=0.999. 86% of non-baseline examples flip within α∈[0.95, 1.0]; zero non-monotonic transitions across all examples. The mechanism holds: `final_layernorm`'s scale invariance collapses W_CE's output norm to ~0.7% of Phi-2's residual norm, shifting the S-curve to α values near 1.0.
13. **Perplexity at operating α — corrected from v2.0.0.** The PPL transition is continuous, not a discrete override. Fine-resolution analysis (73 α points, 5 seeds) on WikiText-2: PPL is flat through α=0.95 (~80.7), then rises smoothly — α=0.98→81.1, α=0.99→83.4, α=0.995→99.8, α=1.0→~10¹⁰. The catastrophic collapse at α=1.0 is real, but it is preceded by a continuous 25% degradation (80.7→99.8). The coarse 5-point grid (α∈{0.0, 0.3, 0.5, 0.7, 1.0}) sampled only the flat region and the endpoint, producing the misleading "discrete override" claim. The corrected picture is a continuous but steep rise, not a jump.
14. **Cross-model comparison with Qwen2-Math invalidated by tokenizer mismatch:** Qwen2-Math's BPE splits 87/97 numbers → only 10 unique lm_head outputs. W_CE logit lens capped at 7.3%. The hypothesis "math-pretrained LLMs resonate better" is untestable via lm_head.
15. **Cross-model probe (bypasses tokenizer):** probe on W_CE-injected L_last converges for all models. At α=0.5: Phi-2=0.9996, Qwen2-Math=0.9848 — W_CE dominates and model-specific differences vanish. Math pretraining does not improve W_CE resonance.
...
## v3.0.0 Correction (2026-07-15)

Findings #10, #12, and #13 above are corrected from their v2.0.0 versions. The original claims — "discontinuous override," "discrete cliff," "abrupt PPL collapse from baseline to 10⁹" — reflected a coarse-grid measurement artifact. Fine-resolution analysis (73 α points, n=2823, 5 seeds) reveals:

- **Accuracy transition is a steep but continuous S-curve,** compressed into Δα ≈ 0.05 (86% of non-baseline examples flip within α∈[0.95, 1.0]). At coarse 5-point resolution (α∈{0.0, 0.3, 0.5, 0.7, 1.0}), the S-curve samples only the flat region and the endpoint, producing an apparent step.
- **Perplexity transition is continuous,** not a discrete jump. PPL rises from ~80.7 (flat through α=0.95) to 99.8 at α=0.995 before the catastrophic collapse to ~10¹⁰ at α=1.0 — a 25% continuous degradation preceding the final collapse, not a clean override.
- **The mechanism (norm mismatch from layernorm-inclusive training) is unchanged.** The correction is only to the measurement: the transition is steep but continuous, sampled at inadequate resolution in v2.0.0.
- All other findings (1–11, 14–19) are unaffected. See `CHANGELOG.md` for the full version history.

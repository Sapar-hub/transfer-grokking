...
13. **Perplexity at operating α.** WikiText-2 last-token PPL across all 5 seeds: α ∈ {0.0, 0.3, 0.5, 0.7} is statistically flat for both operations (63.27 → ~63.3, a <0.1% shift), confirming sub-threshold injection has no measurable effect on language modeling quality — consistent with Finding #12's near-zero effect on task accuracy at the same α. At α=1.0, PPL collapses catastrophically: (5.64 ± 0.87) × 10⁹ for addition and (6.10 ± 0.61) × 10⁸ for multiplication (n=5 for both), roughly 7–8 orders of magnitude above baseline. Combined with Finding #12, this confirms α=1.0 is a discrete override of Phi-2's final-layer computation, not an endpoint of a continuous tradeoff curve — there is no intermediate α that offers a meaningful accuracy/fluency compromise.

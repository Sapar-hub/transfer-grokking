...
12. **L31 patch with W_CE achieves 1.0 at α=1.0, but layernorm fix flattened α<1.0.** W_CE(h_A) injected at Phi-2's last layer (L31) gives perfect accuracy at α=1.0. However, adding final_layernorm to W_CE training (commit 6733597) broke α<1.0: α=0.3 (0.250), α=0.5 (0.265), α=0.7 (0.290) — all near baseline 0.235. Old W_CE (no layernorm) gave α=0.3 (0.490), α=0.5 (0.705), α=0.7 (0.995). W_MSE at L31 does nothing (all α = baseline 0.235). The layernorm fix was correct for logit-lens evaluation but catastrophic for patching. If α<1.0 patching is desired, train W_CE without layernorm.
...
18. **Layernorm "fix" regressed α<1.0 L31 patching.** Adding `final_layernorm` to `train_W_ce` (commit 6733597) made W_CE dependent on layernorm statistics. At α<1.0, mixing with Phi-2's residual stream changes those statistics → alignment breaks. L31 CE at α=0.7 dropped 0.995→0.290, α=0.5 0.705→0.265, α=0.3 0.490→0.250. Logit lens was 1.0 before and after — the fix was harmless for evaluation but catastrophic for patching. Old W_CE (no layernorm) was superior because it learned raw-vector lm_head alignment independent of normalization.
